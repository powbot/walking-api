...
public class PathAnalyzer {
    ...
    public static DestinationDetails furthestReachableTile(List<RSTile> path, PathFindingNode currentPosition){
        ...
        for (int i = path.indexOf(currentPosition.getRSTile()); i < path.size() && i >= 0; i++) {
            ...
            if (current == null){
//                General.println("Current RTCT is null.");
                return null;
            }
            if (i + 1 >= path.size()){
//                General.println("We are at the end of the path from the furthestReachableTile method.");
...
//                General.println("Next node is not loaded and is on screen.");
...
//                General.println("Unable to confirm that the next rtct is movable from the current rtct.");
            ...
            if (!Projection.isInMinimap(new RSTile(nextNode.getX(), nextNode.getY(), nextNode.getPlane()))){
//                General.println("Next node is not clickable.");
//            if(new RSTile(nextNode.getX(), nextNode.getY(), nextNode.getPlane()).isClickable()) {
                furthestReachable = current;
                if (next != null) {
...
//        General.println("Reached end of furthestReachableTile method");
        return null;
    }
...
//            System.out.println("PathAnalyzer attempt to find closest tile in path: " + currentPosition + " " + path);
        ...
        for (int i = path.indexOf(currentPosition.getRSTile()); i < path.size() && i >= 0; i++) {
            ...
            if (!new RSTile(nextNode.getX(), nextNode.getY(), nextNode.getPlane()).getTile().matrix().inViewport(true)){
                furthestReachable = current;
                if (next != null) {
                    return new DestinationDetails(PathState.FURTHEST_CLICKABLE_TILE, current, next);
                }
