...
public class Reachable {
    ...
    public boolean isBlocked(Positionable position){
        RSTile localPosition = position.getPosition().toLocalTile();
...
//        General.println("Checking if we can reach position: " + position);
        position = position.toWorldTile();
//        General.println("Updated position to world tile: " + position);
        RSTile playerPosition = Player.getPosition();
        if (playerPosition.getX() == position.getX() && playerPosition.getY() == position.getY()) {
            return true;
        }
        RSTile localTile = position.toLocalTile();
//        General.println("Updated position to local tile: " + localTile);
...
//        General.println("Checking local tile: " + tile);
        int x = tile.getX(), y = tile.getY();
        if (x < 0 || y < 0) {
            General.println("Tile x or y is below 0");
            return null;
        }
        if (x >= 104 || y >= 104 || x >= map.length || y >= map[x].length){
            General.println("Tile x or y is above 104, or the map length");
            return null;
        }
        return map[x][y];
    }
     ...
     */
    public static RSTile[][] generateMap(RSTile homeTile) {
