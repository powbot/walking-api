...
import dax_api.shared.RSRegion;
...
//        General.println("Collision data: " + collisionData + ", blockedNorth: " + blockedNorth(collisionData) + ", blockedEast: " + blockedEast(collisionData) + ", blockedSouth: " + blockedSouth(collisionData) + ", blockedWest: " + blockedWest(collisionData) + ", isWalkable: " + isWalkable(collisionData
...
//                .check(collisionData, CollisionFlags.INITIALIZED));
    ...
    public static RealTimeCollisionTile get(int x, int y, int z){
        ...
        if (yMap == null){
            General.println("Y map is null.");
            return null;
        }
        HashMap<Integer, RealTimeCollisionTile> zMap = yMap.get(y);
        if (zMap == null){
            General.println("Z map is null.");
            return null;
        }
        return zMap.get(z);
    }
    ...
    public static RealTimeCollisionTile create(int x, int y, int z, int collision){
        ...
        if (!realTimeCollisionTile.isInitialized()){
            General.println("Real time collision tile is not initialized.");
            return null;
        }
