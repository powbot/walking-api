...
public class RSItemHelper {
    ...
    public static boolean clickMatch(RSItem item, String regex){
        General.println("Using regex: " + regex);
    ...
    public static boolean click(Predicate<RSItem> filter, String action){
     ...
     * @return
     */
    public static boolean click(Predicate<RSItem> filter, String action, boolean one){
        if (action == null){
            action = "";
        }
    ...
    public static boolean use(int itemID){
        ...
        String itemName;
        if (Game.getItemSelectionState() == 1 && (itemName = rsItemDefinition.getName()) != null && name.equals(itemName)){
            return true;
    ...
    public static RSItem getItem(Predicate<RSItem> filter){
    ...
    public static int distanceToMouse(RSItem item){
        ...
        return (int) Mouse.getPos().distance(new Point(rectangle.getX() + rectangle.getWidth(), rectangle.getY() + rectangle.getHeight()));
    }
