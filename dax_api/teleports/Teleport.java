...
import dax_api.shared.helpers.magic.Spell;
import dax_api.shared.helpers.magic.SpellBook;
...
public enum Teleport {
	...
	RING_OF_WEALTH_MISCELLANIA(
			...
			(i1, i2) -> WearableItemTeleport.has(WearableItemTeleport.RING_OF_WEALTH_FILTER, i1, i2) && Game.getSetting(359) >= 100,
...
//	),
	PEST_CONTROL_MINIGAME(
			...
			(i1, i2) -> Player.getRSPlayer().getCombatLevel() >= 40
	),
...
//	),
	...
	LUMBRIDGE_HOME_TELEPORT(
			...
			() -> {
				...
				return selectSpell(org.powbot.api.rt4.Magic.Spell.HOME_TELEPORT, "Cast") && Timing.waitCondition(() ->  !Player.getRSPlayer().isInCombat() &&
...
//	),
	...
	Teleport(Grouping.MINIGAMES minigame, RSTile location, String... chatOptions){
		this(minigame, location, null, chatOptions);
	}

	Teleport(Grouping.MINIGAMES minigame, RSTile location, boolean requiresMembers, String... chatOptions){
		this(minigame, location, null, requiresMembers, chatOptions);
	}
	Teleport(Grouping.MINIGAMES minigame, RSTile location, Requirement requirement, String... chatOptions){
		this(minigame, location, requirement, true, chatOptions);
	}
	Teleport(Grouping.MINIGAMES minigame, RSTile location, Requirement requirement, boolean requiresMembers, String... chatOptions){
		setMoveCost(150);
		this.location = location;
		if(requirement != null){
	...
	public boolean canUse(RSItem[] inventory, RSItem[] equipment){
		...
		General.println("[TELEPORTS] " + this + " is available: " + canUse);
		return canUse;
	}
	...
	public static List<RSTile> getValidStartingRSTiles(boolean membersWorld, boolean pvpWorld, List<Teleport> blacklist, RSItem[] inventory, RSItem[] equipment) {
	...
	private static boolean teleportWithScrollInterface(Predicate<RSItem> itemFilter, String regex){
		...
		if(!Interfaces.isInterfaceSubstantiated(TeleportConstants.SCROLL_INTERFACE_MASTER)){
			...
			General.println("Interaction value returned: " + interact);
			if (!interact ||
