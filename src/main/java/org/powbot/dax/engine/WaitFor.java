...
public class WaitFor {
    ...
    public static Condition getNotMovingCondition(){
        return new Condition() {
            ...
            public Return active() {
                if (Timing.timeFromMark(startTime) > movingDelay && initialTile.equals(Players.local().tile()) && !Player.isMoving()) {
                    return Return.FAIL;
                }
    ...
    public static int getMovementRandomSleep(Positionable positionable){
        return getMovementRandomSleep(Players.local().tile().distanceTo(positionable));
    }
