...
public enum Teleport {
	...
	ARDOUGNE_TELEPORT_TAB(
			35, new Tile(2661, 3300, 0),
			(i1, i2) -> Varpbits.varpbit(165) >= 29 && i1.stream().anyMatch(ItemFilters.nameEquals("Ardougne teleport")),
	...
	SLAYER_RING_RELLEKKA_CAVE(
			...
			(i1, i2) -> WearableItemTeleport.has(WearableItemTeleport.SLAYER_RING, i1 , i2),
			() -> WearableItemTeleport.teleport(WearableItemTeleport.SLAYER_RING, ".*Fremennik.*")
	),
...
//	),
	...
	QUETZAL_WHISTLE_ALDARIN(
			...
			(i1, i2) ->  i1.stream().anyMatch(ItemFilters.nameContains("quetzal whistle")) && Quetzal.getInstance().getCurrentCharges() > 0,
	...
	QUETZAL_WHISTLE_CAM_TORUM(
			...
			(i1, i2) ->  Varpbits.value(9955) >= 1 && i1.stream().anyMatch(ItemFilters.nameContains("quetzal whistle")) && Quetzal.getInstance().getCurrentCharges() > 0,
	...
	QUETZAL_WHISTLE_COLOSSAL_WYRM_REMAINS(
			...
			(i1, i2) -> Varpbits.value(9956) >= 1 && i1.stream().anyMatch(ItemFilters.nameContains("quetzal whistle")) && Quetzal.getInstance().getCurrentCharges() > 0,
			() -> Quetzal.teleport(Quetzal.Location.COLOSSAL_WYRM_REMAINS)
	),
	QUETZAL_WHISTLE_FORTIS_COLOSSEUM(
			35, new Tile(1779, 3111, 0),
			(i1, i2) -> Varpbits.value(9958) >= 1 && i1.stream().anyMatch(ItemFilters.nameContains("quetzal whistle")) && Quetzal.getInstance().getCurrentCharges() > 0,
	...
	QUETZAL_WHISTLE_OUTER_FORTIS(
			...
			(i1, i2) -> Varpbits.value(9957) >= 1 && i1.stream().anyMatch(ItemFilters.nameContains("quetzal whistle")) && Quetzal.getInstance().getCurrentCharges() > 0,
	...
	QUETZAL_WHISTLE_SALVAGER_OUTLOOK(
			...
			(i1, i2) -> Varpbits.value(11379) >= 1 && i1.stream().anyMatch(ItemFilters.nameContains("quetzal whistle")) && Quetzal.getInstance().getCurrentCharges() > 0,
			() -> Quetzal.teleport(Quetzal.Location.SALVAGER_OVERLOOK)
	),
	QUETZAL_WHISTLE_SUNSET_COAST(
			35, new Tile(1548, 2995, 0),
			(i1, i2) -> Varpbits.value(9953) >= 1 && i1.stream().anyMatch(ItemFilters.nameContains("quetzal whistle")) && Quetzal.getInstance().getCurrentCharges() > 0,
