...
public enum Teleport {
	...
	VARROCK_TELEPORT(
			...
			Magic.Spell.VARROCK_TELEPORT::canCast,
	...
	VARROCK_TELEPORT_TAB(
			35, new Tile(3212, 3424, 0),
			() -> Inventory.stream().anyMatch(ItemFilters.nameEquals("Varrock teleport")),
			() -> ItemHelper.click("Varrock t.*", "Break")
	),

	VARROCK_TELEPORT_GRAND_EXCHANGE(
			35, new Tile(3161, 3478, 0),
			() -> Magic.Spell.VARROCK_TELEPORT.canCast() && TeleportConstants.isVarrockTeleportAtGE(),
			() -> selectSpell(Spell.VARROCK_TELEPORT,"Grand Exchange")
	),

	LUMBRIDGE_TELEPORT(
			35, new Tile(3225, 3219, 0),
			Magic.Spell.LUMBRIDGE_TELEPORT::canCast,
	...
	LUMBRIDGE_TELEPORT_TAB(
			35, new Tile(3225, 3219, 0),
			() -> Inventory.stream().anyMatch(ItemFilters.nameEquals("Lumbridge teleport")),
			() -> ItemHelper.click("Lumbridge t.*", "Break")
	),

	FALADOR_TELEPORT(
			35, new Tile(2966, 3379, 0),
			Magic.Spell.FALADOR_TELEPORT::canCast,
	...
	FALADOR_TELEPORT_TAB(
			35, new Tile(2966, 3379, 0),
			() -> Inventory.stream().anyMatch(ItemFilters.nameEquals("Falador teleport")),
			() -> ItemHelper.click("Falador t.*", "Break")
	),

	CAMELOT_TELEPORT(
			35, new Tile(2757, 3479, 0),
			Magic.Spell.CAMELOT_TELEPORT::canCast,
	...
	CAMELOT_TELEPORT_TAB(
			35, new Tile(2757, 3479, 0),
			() -> Inventory.stream().anyMatch(ItemFilters.nameEquals("Camelot teleport")),
			() -> ItemHelper.click("Camelot t.*", "Break")
	),

	SEERS_TELEPORT(
			35, new Tile(2757, 3479, 0),
			() -> Magic.Spell.CAMELOT_TELEPORT.canCast() && Varpbits.value(4560, true) == 1,
			() -> selectSpell(Spell.CAMELOT_TELEPORT,"Seers'")
	),

	ARDOUGNE_TELEPORT(
			35, new Tile(2661, 3300, 0),
			Magic.Spell.ARDOUGNE_TELEPORT::canCast,
	...
	ARDOUGNE_TELEPORT_TAB(
			35, new Tile(2661, 3300, 0),
			() -> Inventory.stream().anyMatch(ItemFilters.nameEquals("Ardougne teleport")),
	...
	RING_OF_WEALTH_GRAND_EXCHANGE(
			35, new Tile(3161, 3478, 0),
			() -> WearableItemTeleport.has(WearableItemTeleport.RING_OF_WEALTH_FILTER, Inventory.get(), Equipment.get()),
	...
	RING_OF_WEALTH_MISCELLANIA(
			...
			() -> WearableItemTeleport.has(WearableItemTeleport.RING_OF_WEALTH_FILTER, Inventory.get(), Equipment.get()) && Varpbits.varpbit(359, true) >= 100,
	...
	RING_OF_DUELING_PVP_ARENA (
			...
			() -> WearableItemTeleport.has(WearableItemTeleport.RING_OF_DUELING_FILTER, Inventory.get(), Equipment.get()),
	...
	NECKLACE_OF_PASSAGE_WIZARD_TOWER (
			...
			() -> WearableItemTeleport.has(WearableItemTeleport.NECKLACE_OF_PASSAGE_FILTER, Inventory.get(), Equipment.get()),
	...
	COMBAT_BRACE_WARRIORS_GUILD (
			...
			() -> WearableItemTeleport.has(WearableItemTeleport.COMBAT_BRACE_FILTER, Inventory.get(), Equipment.get()),
	...
	GAMES_NECK_BURTHORPE (
			...
			() -> WearableItemTeleport.has(WearableItemTeleport.GAMES_NECKLACE_FILTER, Inventory.get(), Equipment.get()),
	...
	GAMES_NECK_WINTERTODT (
			...
			() -> hasBeenToZeah() && WearableItemTeleport.has(WearableItemTeleport.GAMES_NECKLACE_FILTER, Inventory.get(), Equipment.get()),
			() -> WearableItemTeleport.teleport(WearableItemTeleport.GAMES_NECKLACE_FILTER, "(?i).*wintertodt.*")
	),

	GLORY_EDGEVILLE (
			35, new Tile(3087, 3496, 0),
			() -> WearableItemTeleport.has(WearableItemTeleport.GLORY_FILTER, Inventory.get(), Equipment.get()),
	...
	SKILLS_FISHING_GUILD (
			...
			() -> WearableItemTeleport.has(WearableItemTeleport.SKILLS_FILTER, Inventory.get(), Equipment.get()),
	...
	SKILLS_WOODCUTTING_GUILD (
			...
			() -> hasBeenToZeah() && WearableItemTeleport.has(WearableItemTeleport.SKILLS_FILTER, Inventory.get(), Equipment.get()),
	...
	SKILLS_FARMING_GUILD_OUTSIDE (
			...
			() -> WearableItemTeleport.has(WearableItemTeleport.SKILLS_FILTER, Inventory.get(), Equipment.get()) && (Varpbits.value(4895, true) < 600 || Skills.realLevel(Skill.Farming) < 45),
	...
	BURNING_AMULET_CHAOS_TEMPLE (
			...
			() -> WearableItemTeleport.has(WearableItemTeleport.BURNING_AMULET_FILTER, Inventory.get(), Equipment.get()),
	...
	DIGSITE_PENDANT_BARGE(
			...
			() -> WearableItemTeleport.has(WearableItemTeleport.DIGSITE_PENDANT_FILTER, Inventory.get(), Equipment.get()),
	...
	ECTOPHIAL (
			...
			() -> Inventory.stream().anyMatch(ItemFilters.nameEquals("Ectophial")),
			() -> Inventory.stream().filter(ItemFilters.nameContains("Ectophial")).first().interact("Empty")
	),

	LLETYA (
			35, new Tile(2330,3172,0),
			() -> WearableItemTeleport.has(WearableItemTeleport.TELEPORT_CRYSTAL_FILTER, Inventory.get(), Equipment.get()),
			() -> WearableItemTeleport.teleport(WearableItemTeleport.TELEPORT_CRYSTAL_FILTER, "Lletya")
	),

	XERICS_GLADE(
			35, new Tile(1753, 3565, 0),
			() -> WearableItemTeleport.has(WearableItemTeleport.XERICS_TALISMAN_FILTER, Inventory.get(), Equipment.get()),
	...
	WEST_ARDOUGNE_TELEPORT_TAB(
			...
			() -> Inventory.stream().name("West ardougne teleport").count(true) > 0,
			() -> ItemHelper.click("West ardougne t.*", "Break")
	),

	RADAS_BLESSING_KOUREND_WOODLAND(
			0, new Tile(1558, 3458, 0),
			() -> WearableItemTeleport.has(WearableItemTeleport.RADAS_BLESSING_FILTER, Inventory.get(), Equipment.get()),
	...
	RADAS_BLESSING_MOUNT_KARUULM(
			...
			() -> WearableItemTeleport.has(WearableItemTeleport.RADAS_BLESSING_FILTER.and(ItemFilters.nameContains("3","4")), Inventory.get(), Equipment.get()),
	...
	CRAFTING_CAPE_TELEPORT(
			...
			() -> WearableItemTeleport.has(WearableItemTeleport.CRAFTING_CAPE_FILTER, Inventory.get(), Equipment.get()),
	...
	CABBAGE_PATCH_TELEPORT(
			...
			() -> WearableItemTeleport.has(WearableItemTeleport.EXPLORERS_RING_FILTER, Inventory.get(), Equipment.get()),
	...
	LEGENDS_GUILD_TELEPORT(
			...
			() -> WearableItemTeleport.has(WearableItemTeleport.QUEST_CAPE_FILTER, Inventory.get(), Equipment.get()),
	...
	KANDARIN_MONASTERY_TELEPORT(
			...
			() -> WearableItemTeleport.has(WearableItemTeleport.ARDOUGNE_CLOAK_FILTER, Inventory.get(), Equipment.get()),
	...
	RIMMINGTON_TELEPORT_TAB(
			...
			() -> Inventory.stream().anyMatch(ItemFilters.nameEquals("Rimmington teleport")),
			() -> ItemHelper.click("Rimmington t.*", "Break")
	),

	TAVERLEY_TELEPORT_TAB(
			35, new Tile(2894, 3465, 0),
			() -> Inventory.stream().anyMatch(ItemFilters.nameEquals("Taverly teleport")),
			() -> ItemHelper.click("Taverley t.*", "Break")
	),

	RELLEKKA_TELEPORT_TAB(
			35, new Tile(2668, 3631, 0),
			() -> Inventory.stream().anyMatch(ItemFilters.nameEquals("Rellekka teleport")),
			() -> ItemHelper.click("Rellekka t.*", "Break")
	),

	BRIMHAVEN_TELEPORT_TAB(
			35, new Tile(2758, 3178, 0),
			() -> Inventory.stream().anyMatch(ItemFilters.nameEquals("Brimhaven teleport")),
			() -> ItemHelper.click("Brimhaven t.*", "Break")
	),

	POLLNIVNEACH_TELEPORT_TAB(
			35, new Tile(3340, 3004, 0),
			() -> Inventory.stream().anyMatch(ItemFilters.nameEquals("Pollnivneach teleport")),
			() -> ItemHelper.click("Pollnivneach t.*", "Break")
	),

	YANILLE_TELEPORT_TAB(
			35, new Tile(2544, 3095, 0),
			() -> Inventory.stream().anyMatch(ItemFilters.nameEquals("Yanille teleport")),
			() -> ItemHelper.click("Yanille t.*", "Break")
	),

	HOSIDIUS_TELEPORT_TAB(
			35, new Tile(1744, 3517, 0),
			() -> Inventory.stream().anyMatch(ItemFilters.nameEquals("Hosidius teleport")),
			() -> ItemHelper.click("Hosidius t.*", "Break")
	),

	CONSTRUCTION_CAPE_RIMMINGTON(
			0, new Tile(2954,3224, 0),
			() -> WearableItemTeleport.has(WearableItemTeleport.CONSTRUCTION_CAPE_FILTER, Inventory.get(), Equipment.get()),
	...
	SLAYER_RING_GNOME_STRONGHOLD(
			...
			() -> WearableItemTeleport.has(WearableItemTeleport.SLAYER_RING, Inventory.get(), Equipment.get()),
	...
	SALVE_GRAVEYARD_TAB(
			...
			() -> Inventory.stream().anyMatch(ItemFilters.nameEquals("Salve graveyard teleport")),
			() -> ItemHelper.click("Salve graveyard t.*", "Break")
	),

	FENKENSTRAINS_CASTLE_TAB(
			35, new Tile(3547, 3528, 0),
			() -> Inventory.stream().anyMatch(ItemFilters.nameEquals("Fenkenstrain's castle teleport")),
			() -> ItemHelper.click("Fenkenstrain's castle t.*", "Break")
	),

	BARROWS_TAB(
			35, new Tile(3565, 3314, 0),
			() -> Inventory.stream().anyMatch(ItemFilters.nameEquals("Barrows teleport")),
			() -> ItemHelper.click("Barrows t.*", "Break")
	),

	ARCEUUS_LIBRARY_TAB(
			35, new Tile(1632, 3838, 0),
			() -> Inventory.stream().anyMatch(ItemFilters.nameEquals("Arceuus library teleport")),
			() -> ItemHelper.click("Arceuus library t.*", "Break")
	),

	BATTLEFRONT_TAB(
			35, new Tile(1349,3738,0),
			() -> Inventory.stream().anyMatch(ItemFilters.nameEquals("Battlefront teleport")),
			() -> ItemHelper.click("Battlefront t.*", "Break")
	),

	DRAYNOR_MANOR_TAB(
			35, new Tile(3109,3352,0),
			() -> Inventory.stream().anyMatch(ItemFilters.nameEquals("Draynor manor teleport")),
			() -> ItemHelper.click("Draynor manor t.*", "Break")
	),

	MIND_ALTAR_TAB(
			35, new Tile(2980, 3510, 0),
			() -> Inventory.stream().anyMatch(ItemFilters.nameEquals("Mind altar teleport")),
			() -> ItemHelper.click("Mind altar t.*", "Break")
	),

	ENCHANTED_LYRE_RELLEKA(
			35, new Tile(2661, 3465, 0),
			() -> Inventory.stream().anyMatch(ItemFilters.nameContains("Enchanted lyre")),
			() -> {
	...
	FARMING_CAPE_TELEPORT(
			...
			() -> WearableItemTeleport.has(WearableItemTeleport.FARMING_CAPE_FILTER, Inventory.get(), Equipment.get()),
	...
	ARDOUGNE_FARM_TELEPORT(
			...
			() -> WearableItemTeleport.has(WearableItemTeleport.ARDOUGNE_CLOAK_4_FILTER, Inventory.get(), Equipment.get()),
			() -> WearableItemTeleport.teleport(WearableItemTeleport.ARDOUGNE_CLOAK_4_FILTER, "Farm Teleport")
	),

	ROYAL_SEED_POD(
			0, new Tile(2465, 3495, 0),
			() -> Inventory.stream().anyMatch(ItemFilters.nameEquals("Royal seed pod")),
	...
	DRAKANS_MEDALLION_VER_SINHAZA(
			...
			() -> WearableItemTeleport.has(WearableItemTeleport.DRAKANS_MEDALLION_FILTER, Inventory.get(), Equipment.get()),
	...
	FISHING_TRAWLER_MINIGAME(
			...
			() -> Skills.realLevel(Skill.Fishing) >= 15
	),
...
//	),
	PEST_CONTROL_MINIGAME(
			...
			() -> Players.local().getCombatLevel() >= 40
	),
...
//	),
	...
	POH_OUTSIDE_RIMMINGTON(
			35, new Tile(2954, 3224, 0),
			() -> POH.RIMMINGTON.isHouseLocation() && Magic.Spell.TELEPORT_TO_HOUSE.canCast(),
			() -> Spell.TELEPORT_TO_HOUSE.cast("Outside")
	),
	POH_OUTSIDE_RIMMINGTON_TAB(
			35, new Tile(2954, 3224, 0),
			() -> POH.RIMMINGTON.isHouseLocation() && Inventory.stream().anyMatch(ItemFilters.nameEquals("Teleport to house")),
			() -> ItemHelper.click("Teleport to house", "Outside")
	),
	POH_OUTSIDE_TAVERLY(
			35, new Tile(2894, 3465, 0),
			() -> POH.TAVERLY.isHouseLocation() && Magic.Spell.TELEPORT_TO_HOUSE.canCast(),
			() -> Spell.TELEPORT_TO_HOUSE.cast("Outside")
	),
	POH_OUTSIDE_TAVERLY_TAB(
			35, new Tile(2894, 3465, 0),
			() -> POH.TAVERLY.isHouseLocation() && Inventory.stream().anyMatch(ItemFilters.nameEquals("Teleport to house")),
			() -> ItemHelper.click("Teleport to house", "Outside")
	),
	POH_OUTSIDE_POLLNIVNEACH_TAB(
			35, new Tile(3340, 3004, 0),
			() -> POH.POLLNIVNEACH.isHouseLocation() && Magic.Spell.TELEPORT_TO_HOUSE.canCast(),
			() -> Spell.TELEPORT_TO_HOUSE.cast("Outside")
	),
	POH_OUTSIDE_POLLNIVNEACH(
			35, new Tile(3340, 3004, 0),
			() -> POH.POLLNIVNEACH.isHouseLocation() && Inventory.stream().anyMatch(ItemFilters.nameEquals("Teleport to house")),
			() -> ItemHelper.click("Teleport to house", "Outside")
	),
	POH_OUTSIDE_HOSIDIUS(
			35, new Tile(1744, 3517, 0),
			() -> POH.HOSIDIUS.isHouseLocation() && Magic.Spell.TELEPORT_TO_HOUSE.canCast(),
			() -> Spell.TELEPORT_TO_HOUSE.cast("Outside")
	),
	POH_OUTSIDE_HOSIDIUS_TAB(
			35, new Tile(1744, 3517, 0),
			() -> POH.HOSIDIUS.isHouseLocation() && Inventory.stream().anyMatch(ItemFilters.nameEquals("Teleport to house")),
			() -> ItemHelper.click("Teleport to house", "Outside")
	),
	POH_OUTSIDE_RELLEKKA(
			35, new Tile(2670, 3632, 0),
			() -> POH.RELLEKKA.isHouseLocation() && Magic.Spell.TELEPORT_TO_HOUSE.canCast(),
			() -> Spell.TELEPORT_TO_HOUSE.cast("Outside")
	),
	POH_OUTSIDE_RELLEKKA_TAB(
			35, new Tile(2670, 3632, 0),
			() -> POH.RELLEKKA.isHouseLocation() && Inventory.stream().anyMatch(ItemFilters.nameEquals("Teleport to house")),
			() -> ItemHelper.click("Teleport to house", "Outside")
	),
	POH_OUTSIDE_BRIMHAVEN(
			35, new Tile(2750, 3178, 0),
			() -> POH.BRIMHAVEN.isHouseLocation() && Magic.Spell.TELEPORT_TO_HOUSE.canCast(),
			() -> Spell.TELEPORT_TO_HOUSE.cast("Outside")
	),
	POH_OUTSIDE_BRIMHAVEN_TAB(
			35, new Tile(2750, 3178, 0),
			() -> POH.BRIMHAVEN.isHouseLocation() && Inventory.stream().anyMatch(ItemFilters.nameEquals("Teleport to house")),
			() -> ItemHelper.click("Teleport to house", "Outside")
	),
	POH_OUTSIDE_YANILLE(
			35, new Tile(2544, 3095, 0),
			() -> POH.YANILLE.isHouseLocation() && Magic.Spell.TELEPORT_TO_HOUSE.canCast(),
			() -> Spell.TELEPORT_TO_HOUSE.cast("Outside")
	),
	POH_OUTSIDE_YANILLE_TAB(
			35, new Tile(2544, 3095, 0),
			() -> POH.YANILLE.isHouseLocation() && Inventory.stream().anyMatch(ItemFilters.nameEquals("Teleport to house")),
			() -> ItemHelper.click("Teleport to house", "Outside")
	),
	POH_OUTSIDE_PRIFDDINAS(
			35, new Tile(3239, 6076, 0),
			() -> POH.PRIFDDINAS.isHouseLocation() && Magic.Spell.TELEPORT_TO_HOUSE.canCast(),
			() -> Spell.TELEPORT_TO_HOUSE.cast("Outside")
	),
	POH_OUTSIDE_PRIFDDINAS_TAB(
			35, new Tile(3239, 6076, 0),
			() -> POH.PRIFDDINAS.isHouseLocation() && Inventory.stream().anyMatch(ItemFilters.nameEquals("Teleport to house")),
			() -> ItemHelper.click("Teleport to house", "Outside")
	),

	KHARYRLL_TELEPORT_TAB(
			35, new Tile(3496, 3472, 0),
			() -> hasCompletedDesertTreasure() && Inventory.stream().anyMatch(ItemFilters.nameEquals("Kharyrll teleport")),
			() -> ItemHelper.click("Kharyrll t.*", "Break")
	),

	LASSAR_TELEPORT_TAB(
			35, new Tile(3001, 3470, 0),
			() -> hasCompletedDesertTreasure() && Inventory.stream().anyMatch(ItemFilters.nameEquals("Lassar teleport")),
...
//	),
	...
	Teleport(int moveCost, Tile location, Supplier<Boolean> requirement, Action action) {
		this(moveCost, location, requirement, action, true);
	}

	Teleport(int moveCost, Tile location, Supplier<Boolean> requirement, Action action, boolean requiresMembers) {
	...
	Teleport(int moveCost, Tile location, Supplier<Boolean> requirement, Action action, TeleportLimit limit) {
		this(moveCost, location, requirement, action, limit, true);
	}

	Teleport(int moveCost, Tile location, Supplier<Boolean> requirement, Action action, TeleportLimit limit, boolean requiresMembers) {
		setMoveCost(moveCost);
		this.location = location;
		this.requirement = requirement;
		this.action = action;
	...
	Teleport(Grouping.MINIGAMES minigame, Tile location, Supplier<Boolean> requirement, String... chatOptions){
		this(minigame, location, requirement, true, chatOptions);
	}
	Teleport(Grouping.MINIGAMES minigame, Tile location, Supplier<Boolean> requirement, boolean requiresMembers, String... chatOptions){
		setMoveCost(75);
		this.location = location;
		if(requirement != null){
			this.requirement = () -> canUseMinigameTeleport() && requirement.get();
		} else{
		...
		return Arrays.stream(values())
				.filter(t -> {
					if((!membersWorld && t.requiresMembers) || (pvpWorld && !t.canBeUsedInPvpWorlds))
						return false;
					return !blacklist.contains(t) && t.teleportLimit.canCast(wildernessLevel) && t.requirement.get();
				})
