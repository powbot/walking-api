...
//        Mouse.sendClickEvent(point, button);
    ...
    public static boolean click(Clickable07 clickable, String... clickActions) {
        return action(clickable, false, clickActions);
    }

    public static boolean hover(Clickable07 clickable, String... clickActions) {
    ...
    public static boolean clickMinimap(Positionable tile) {
        ...
        for (int i = 0; i < General.random(7, 10); i++) {
            ...
            Movement.step(tile.getPosition().getTile());
...
////            }
    ...
    public static boolean action(Clickable07 clickable, boolean hover, String... clickActions) {
        if (clickable == null) {
            return false;
        }
        String name = null;
        ...
        for (int i = 0; i < General.random(3, 5); i++) {
            ...
            if (WaitFor.condition(100, () -> {
                ...
                if (waitResponse() == State.YELLOW) {
                    ...
                    return clicked != null && (clicked.equals(destination) || Players.local().tile().equals(destination));
                } else {
                    break;
                }
            }
     ...
     */
    private static boolean attemptAction(RSModel model, Clickable07 clickable, String targetName, boolean hover, String... clickActions) {
...
//        return false;
    ...
    private static boolean handleRSItemRSInterface(Clickable07 clickable, boolean hover, String... clickActions) {
        return clickable.click(clickActions);
...
//    }
    ...
    private static RSMenuNode getValidMenuNode(Clickable07 clickable, String targetName, RSMenuNode[] menuNodes, String... clickActions) {
    ...
    public static Point getWalkingPoint(RSTile destination) {
        ...
        for (int i = 0; i < 1000; i++) {
            Point point = getRandomPoint(area.getPolygon().getBounds());
            if (Projection.isInViewport(point) && area.getPolygon().contains(point, false)) {
    ...
    private static boolean isWalkingPoint(Point point, Tile destination) {
        ...
        if (!area.getPolygon().contains(point, false)) {
            return false;
        }
    ...
    private static Point getRandomPoint(Rectangle rectangle) {
        return new Point(getRandomInteger(rectangle.getX(), rectangle.getX() + rectangle.getWidth()), getRandomInteger(rectangle.getY(), rectangle.getY() + rectangle.getHeight()));
    }
...
//        }
//        for (RSGroundItem rsGroundItem : GroundItems.find(rsGroundItem -> rsGroundItem.getPosition().equals(tile))) {
